...
class BambuColorVoxelizerApp:
    ...
    def _add_object_controls(self, parent: ttk.Frame, row: int) -> int:
        ...
        grid_spin.grid(row=0, column=1, sticky="ew", pady=2, padx=(8, 0))
        ...
        btn_02 = ttk.Button(preset_frame, text="0.2mm Nozzle", command=lambda: self.apply_nozzle_preset(0.2))
        btn_02.grid(row=0, column=0, padx=(0, 2), sticky="ew")

        btn_04 = ttk.Button(preset_frame, text="0.4mm Nozzle", command=lambda: self.apply_nozzle_preset(0.4))
        btn_04.grid(row=0, column=1, padx=(2, 0), sticky="ew")
    ...
    def apply_nozzle_preset(self, nozzle_size: float) -> None:
        try:
            max_x = float(self.max_x_var.get())
            if max_x <= 0:
                raise ValueError()
        except ValueError:
            messagebox.showerror("Error", "Please enter a valid positive Max Size X first.")
            return
        ...
        calculated_res = int(max(1, min(600, round(max_x / voxel_size))))
        self.grid_resolution_var.set(calculated_res)
        self.schedule_preview_update(immediate=True)
        self.status_var.set(
            f"Set resolution to {calculated_res} for {nozzle_size}mm nozzle "
            f"(voxel width: {max_x / calculated_res:.3f} mm)."
        )
    ...
    def _bind_live_preview_vars(self) -> None:
        for variable in (
            self.max_x_var, self.max_y_var, self.corner_radius_var, self.frame_enabled_var,
            self.frame_width_var, self.grid_resolution_var, self.base_thickness_var, self.color_thickness_var,
        ):
